...
class Encoder(nn.Module):
    def __init__(self, input_shape=(96, 84), hid_channels=16):
        super().__init__()
        self.Input = Reshape(dims=[input_shape, 1, 1])

        self.Base = nn.Sequential()
        self.Base.add_module('conv0', nn.Conv2d(input_shape, hid_channels,
                                                kernel_size=(1, 12), stride=(1, 12), padding=0))
        self.Base.add_module('bn0', nn.BatchNorm2d(hid_channels))
        self.Base.add_module('lrelue0', nn.LeakyReLU(0.3))

        self.Base.add_module('conv1', nn.Conv2d(input_shape, hid_channels,
                                                kernel_size=(1, 7), stride=(1, 7), padding=0))
        self.Base.add_module('bn1', nn.BatchNorm2d(hid_channels))
        self.Base.add_module('lrelue1', nn.LeakyReLU(0.3))

        self.Base.add_module('conv2', nn.Conv2d(input_shape, hid_channels,
                                                kernel_size=(3, 1), stride=(3, 1), padding=0))
        self.Base.add_module('bn2', nn.BatchNorm2d(hid_channels))
        self.Base.add_module('lrelue2', nn.LeakyReLU(0.3))

        self.Base.add_module('conv3', nn.Conv2d(input_shape, hid_channels,
                                                kernel_size=(2, 1), stride=(2, 1), padding=0))
        self.Base.add_module('bn3', nn.BatchNorm2d(hid_channels))
        self.Base.add_module('lrelue3', nn.LeakyReLU(0.3))

        self.Base.add_module('conv4', nn.Conv2d(input_shape, hid_channels,
                                                kernel_size=(2, 1), stride=(2, 1), padding=0))
        self.Base.add_module('bn4', nn.BatchNorm2d(hid_channels))
        self.Base.add_module('lrelue4', nn.LeakyReLU(0.3))

        self.Base.add_module('conv5', nn.Conv2d(input_shape, hid_channels,
                                                kernel_size=(2, 1), stride=(2, 1), padding=0))
        self.Base.add_module('bn5', nn.BatchNorm2d(hid_channels))
        self.Base.add_module('lrelue5', nn.LeakyReLU(0.3))
